...
image::devspaces-arch1.png[title=Red Hat OpenShift Dev Spaces Architecture (High Level)]
...
Each Red Hat OpenShift Dev Spaces workspace starts with a specific editor and associated IDE extensions. Extensions are typically sourced through Open VSX registries, while the embedded plug-in registry is deprecated.
...
Provides a default preconfigured workspace image with common developer tools.
